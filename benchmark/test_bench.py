...
from hetu.data.backend import RedisRawComponentTable, RedisBackend, RaceCondition
import websockets
...
async def bench_sys_call_routine(address, duration, name: str, pid: str, packet):
    ...
    if address.startswith("wss://"):
        ...
        ssl_context.verify_mode = (
            ssl.CERT_NONE
        )  # 应该使用ssl_context.load_cert_chain(cert_file)更安全
    else:
        ssl_context = None
        ...
        async with websockets.connect(address, ssl=ssl_context) as ws:
            ...
            while True:
                ...
                if random.randint(0, 100) == 0:
                    print(
                        f"{pid}号客户端收到数据", len(recv_count), recv_count[cur_min]
                    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--inst_name", default="bench1", help="pubsub bench用，hetu实例名称"
    )
    ...
    match args.item:
        case "call":
            all_results = [
                ...
                run_bench(bench_just_select, args, "select"),
            ]
        case "pubsub":
            # 再开一个写入进程
            ...
            p_uper.start()
            # 开始等待消息
            all_results = [run_bench(bench_pubsub_routine, args, "pubsub")]
        case "direct":
            all_results = [
