...
class CryptoLayer(MessageProcessLayer):
    ...
    def handshake(self, message: bytes) -> tuple[Any, bytes]:
        ...
        """
        assert self._public_key and self._private_key
        ...
        try:
            ...
            shared_point = nacl.bindings.crypto_scalarmult(
                self._private_key.encode(),  # 转为 bytes
            ...
            pub = self._public_key.encode()
            self._private_key = None  # 握手完成后销毁私钥，确保前向安全
            self._public_key = None
            return session_key, pub
