...
class ZstdLayer(MessageProcessLayer, alias="zstd"):
    ...
    def initial_samples(self) -> list[bytes]:
        ...
        def make_rand_sub_message(_comp: type[BaseComponent]):
            ...
            # 字符串列不能留随机字节：UTF-32 会填出超出 Unicode 范围的码位（numpy 造出坏掉的
            # str，编码出无效 UTF-8）和代理字符（编码失败）。改填随机长度的可打印 ASCII
            assert dt.names
            for name in dt.names:
                if dt[name].kind == "U":
                    length = int(rng.integers(0, dt[name].itemsize // 4 + 1))
            ...
            for _ in range(200):
                ...
                if self._parent:
                    try:
                        ...
                        continue  # 编码不了就不要这个样本，样本必须全是 bytes
                else:
