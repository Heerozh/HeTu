    ...
    for table_prefix, rows in pairs(insert_rows) do
        ...
        for _, row in ipairs(rows) do
            ...
            if redis_call('EXISTS', key) == 1 then
                return "RACE: Try insert but key exists: " .. key
            end
    ...
    for table_prefix, rows in pairs(update_rows) do
        ...
        for _, row in ipairs(rows) do
            ...
            if #old_row_raw == 0 then
                return "RACE: Try update key but not exists: " .. key
            end
            ...
            local payload_ver = row["_version"]
            local db_ver = old_row["_version"]
            if db_ver ~= payload_ver then
                return table_concat({
                    "RACE: Optimistic lock failure(update): ", key,
                    " DB=", db_ver, " Req=", payload_ver
                })
            end

            -- 检查 2: 获取改变的字段，和Unique 约束检查 (仅检查被修改的字段)
            for field, new_val in pairs(row) do
                if field ~= "id" and field ~= "_version" then
                    -- 报错未改变的字段
                    if new_val == old_row[field] then
                        error("ASSERT: Commit unchanged value: " .. key ..
...
if delete_rows then
    for table_prefix, rows in pairs(delete_rows) do
        ...
        for _, row in ipairs(rows) do
            ...
            if #old_row_raw == 0 then
                -- 如果已经不存在，说明冲突
                return "RACE: Try delete key but not exists: " .. key
            end
            ...
            if db_ver ~= payload_ver then
                return table_concat({
                    "RACE: Optimistic lock failure (delete): ", key, " DB=", db_ver,
                    " Req=", payload_ver
                })
            end
