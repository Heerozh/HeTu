...
def generate_component(component_cls: type[BaseComponent]):
    attributes = [f"    public {dtype_to_csharp(prop.dtype)} {name};"
                  for name, prop in component_cls.properties_ if name != 'id']

    lines = [
        "",
        f"class {component_cls.component_name_}: IBaseComponent",
        "{",
        f"    public long id {{ get; set; }}",
        *attributes,
        "}",
        "",
    ]
