...
工作器 ID 由 `WorkerKeeper` 自动租用：Redis 后端的租约存为 Redis 键，每 5 秒续租一次，如果进程在未释放租约的情况下死亡，该槽位会在租约过期后被回收；SQL 后端（仅用于开发）直接使用进程在本机的序号。启动时，引擎会从 `WorkerLease` `Component`（一个 `core`/易失性表）恢复*上次持久化的时间戳*，如果系统时钟回退，会等待一个短暂的宽限期——这是 HeTu 在主机重启后 NTP 调整时防止重复 ID 的防御机制。
