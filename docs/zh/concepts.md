...
**`系统`** 是在事务内操作这些表的异步函数。没有继承、没有按行方法、也没有中央“世界”对象。状态存储在 Redis（开发时可用 SQLite）中；系统是无状态的。
...
- **SQLite 只用于开发。** SQLite 后端在本地库文件上模拟 Redis 的行为（索引排序、提交校验、订阅通知都与 Redis 一致），免装服务、方便调试，但不考虑性能；生产请用 Redis。
...
HeTu 使用乐观并发。每个会话维护一个它所读取或写入的行的 `IdentityMap`。提交时，引擎会针对 Redis 检查每个行的版本，以及每次 `range` 读过的区间（区间里新增了行也算变化）。如果底层有任何变化，提交会因 `RaceCondition` 而中止，引擎会自动从顶层**重新运行 `系统`**，最多重试 `retry=` 次（默认 9999）。
...
在后台，`SubscriptionBroker` 监视 Redis 的发布/订阅以获取行更改，根据客户端的权限级别过滤它们，并通过 WebSocket 推送增量。推送按 `1/UPDATE_FREQUENCY`（默认 100ms）合批，一次变更通常在 100～200ms 内到达客户端。

订阅受与 `系统` 相同的权限系统检查，因此客户端无法订阅其无权查看的数据。

**订阅是尽力而为的最终一致。** 正常负载下约 99% 的情况，客户端收到的都是最新数据；Redis 压力过大
（副本复制延迟超过约 100ms）时，客户端可能残留旧数据，直到该行下次变更。原因是通知只说"这行变了"、
不带内容，服务端收到后去随机副本重读，而发通知的节点和被读的副本可能不是同一个：服务端在每条通知
之后（以及订阅生效、断线重连之后）都至少隔一个合批间隔再读一次，副本的复制延迟在这个间隔以内，
读到的就一定是最新值。需要强一致的判断（扣钱、发奖、校验）请放在 `System` 里读——写事务有乐观锁
兜底——不要依赖客户端手里的订阅数据。
...
高频写入的表（位置、血量）请继续用 `range`。服务端用 `MAX_TABLE_SUBSCRIPTION_ROWS`
（默认 10 万行）兜底，超过的表拒绝整表订阅；每连接的数量上限是 `MAX_TABLE_SUBSCRIPTION`。

整表订阅对 RLS 权限的得失都会做出反应（`range` 只对查询范围内的行如此）：失去权限的行推删除，
重新获得权限的行推添加。
