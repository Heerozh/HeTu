...
* `System` 内部的写入可能会发生事务冲突；设置 `retry > 0` 会自动重新运行该 `System`。
    ...
    - 任何已被读取或即将被写入的行，如果被其他进程/协程在底层更改，则会引发冲突。只读过、未修改的行同样计入：
      提交时会校验它们的版本号，因此事务不会建立在陈旧读之上。
        - `range()` 读过的区间同样会在提交时校验：其他 System 往查询区间里**新增**一行（幻读）、删除返回的行、
          或把它的索引字段改到区间外，都会引发冲突；这次读到的是还没同步的副本也一样。所以"`range` 查不到就插入、
          查到就修改"的写法是安全的（SQL 后端挡不住两个事务同时提交，见[查不到就插入](advanced.md#查不到就插入两种写法)）。截断读（返回行数等于
          `limit`）只保护读到的前 `limit` 行，用 `range` 判断"有没有"时要读全（`limit=-1`）。读写频繁的区间、
          且逻辑不依赖"区间里没有别的行"时，传 `phantom_check=False` 关掉区间校验，避免无谓的重试。
