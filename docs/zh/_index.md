...
HeTu (河图) 是一个高性能、多进程、分布式的游戏服务器引擎。它采用 **实体-组件-系统 (ECS)**
模式，将状态存储在 **Redis** 中，并通过 **WebSocket** 以可订阅、行级权限的数据库形式向游戏客户端暴露这些状态。
...
- **有状态的长连接。** 与专注于无状态 CRUD 的后端即服务产品不同，HeTu
  专为内存中的游戏状态而构建，支持毫秒级的推送更新。
- **Redis 吞吐量。** 写入吞吐量大约是典型基于 Postgres 的 BaaS 堆栈的 10 倍；请参阅项目
  README 中的基准测试。
...
这就是完整的交互：一个类型化的表、一个 RPC 入口点和一个响应式订阅。无需模式迁移、API
网关或消息代理。
...
* HeTu 保证客户端对 `System` 的调用按客户端发出的顺序执行；来自同一客户端的每次调用都在同一个协程内运行。
    ...
    - `async` / `await` 不会破坏此保证——当 `await` 挂起时，控制权只能切换到属于*其他*
      连接的协程。
...
* `System` 内部的写入可能会发生事务冲突；设置 `retry > 0` 会自动重新运行该 `System`。
    ...
    - 任何已被读取或即将被写入的行，如果被其他进程/协程在底层更改，则会引发冲突。只读过、未修改的行同样计入：
      提交时会校验它们的版本号，因此事务不会建立在陈旧读之上。
        - 例外：`range()` 锁定的是查询返回的那些行，而不是索引本身。其他 System 往查询区间里**新增**的行属于幻读，
          版本校验看不到它，不会引发冲突；而查询实际返回的那些行和普通读取一样会被校验——删除其中一行、或把它的
          索引字段改到区间外，都会引发冲突。防幻读请使用 `unique` 约束代替索引检查。
        - unique 冲突在提交时检查：本事务曾 `get`（或等值 `range`）观察其不存在的（如 `upsert` 的锚定字段）
          判为竞态、自动重试；否则以 `UniqueViolation` 失败、不重试。要在事务内对"已存在"分支处理、或靠扫描
          推导 unique 值再写入，先 `get` 该值再写。
    - 任何外部或持久化的副作用——更新 `ctx` 全局变量、写入文件、发送网络调用——必须在事务提交
      **之后**发生。否则，当事务被丢弃时，这些副作用可能已经生效。使用显式的
      `ctx.session_commit()` 提前提交；如果发生冲突，它会引发异常，因此其下方的任何代码都不会执行。
    - 使用 `ctx.race_count` 来了解当前运行是第几次重试尝试。
* 事务读的新鲜度
    - 本工作进程有客户端订阅着的行常驻进程内存（worker 行缓存），事务读命中不打数据库，commit 后新行直接写穿进缓存，订阅推送也从这份缓存取行——客户端看到的行与服务端事务读到的永远是同一份。
    - 缓存里的行不早于本进程最近处理的该行变更通知（通知由 commit 发出并携带版本号），所以只读的 `System` 可能读到“通知尚未到达”的旧值，窗口是毫秒级、与客户端订阅推送同等新鲜；写入的 `System` 不受影响，提交时的版本校验会把陈旧读判成冲突并重试。需要强一致读时用 `only_master`（headless 客户端默认如此）。
* 如何减少事务冲突
    - `System` 运行越慢，其冲突概率越高。如果需要在 `System` 内部进行大量计算，请考虑使用
      `Endpoint` 以获得更精细的控制。
    - 关注慢日志；只有在冲突堆积时才深入排查。
