...
### 3.4 `RowSubscription` 按 `_version` 去重

补读与尾随重读多数时候读回来的是客户端已有的数据，不去重的话推送流量会翻倍：

- `RowSubscription` 记 `version`：客户端当前持有的该行 `_version`（行不存在为 None，未知为
  `-1`）。`get_updated` 读回的版本与它**相等**就不推，否则推并更新。
- 初始值：`subscribe_get` 读完初始行后写入（初始读之前登记的订阅是"未知"，期间 tick 推的会被
  客户端丢弃，所以以初始读回的版本为准）；`subscribe_range` 的初始行、`IndexSubscription`
  新进入范围的行都带着读回的版本创建。
- **只去重相等，不拦"更低"**：同 id 删除后重插时 `_version` 从 1 重来，拦更低会把新一代的行
  一直挡在外面。满足延迟预算时读不会倒退（§4），不需要拦。
- 整表订阅不按行记版本（`known_ids` 已是每连接一份，再记版本内存翻倍），尾随重读涉及的那几行
  可能重复推一次，只限迟到消息里的 row_id。
...
去重的正确性：只在"读回版本 == 客户端持有版本"时不推。满足预算时，最后那次读回的就是 W 之后的
状态；若它与客户端持有的版本相等，客户端已经是最新。

已知残留（不处理，记录在此）：
- 复制延迟超过 T：旧值可能残留到该行下次变更——这正是 §3.6 写明的那 ~1%。
- 同 id 删除后重插、新一代恰好走到与客户端持有的相同版本号、且都落在同一次读的间隔内：被当作
  没变而不推。需要显式 id 的插入 + 一个 interval 内至少三次写，概率极低。
...
- 推送流量：补读、尾随重读结果相同的不推（行 / 索引订阅），不增加客户端流量。
...
- 去重：同一版本不重复推；`get_updates(timeout)` 在只有空批时等满超时返回 `{}`。
...
3. `test(sub)` red → `fix(sub)`：`RowSubscription` 版本去重、`get_updates` 跳过空批（§3.4、§3.5）。
