...
3. **值频道上的 `IndexSubscription` 靠行频道发现"离开"**：
   ...
   - RLS 不可见的行客户端没有（`RowSubscription.pushed` 记 None，同行不存在）：它在不可见期间
     变化、或离开，都不推 None——推了等于把不可见行的 id 告诉客户端；变得可见时照推。
...
- 订阅生效后的补读（`request_reread`）不变。pubsub 重订后的 `RESYNC` 同样会让行频道重读、触发
  成员检查，结果无害。
- 行进入结果时是先读行、tick 末尾才订它的行频道：其间的写入没有通知，而值频道不再发"离开"，
  行在这段间隙被删 / 改走就再也发现不了。所以本连接新订上的行频道同样隔一个 interval 补读一次
  （读回一样不推），离开由上表的行频道检查发现。
...
- 改声明需要 `hetu upgrade`（schema 版本变了）。dtype 没变时迁移脚本判 skip，upgrade 只改写
  meta，索引定义变了（`point_sub` 会打开 index）就按新定义重建索引；SQL 后端不会给已有的表
  补建索引或唯一约束。
