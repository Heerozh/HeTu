...
- [ ] 基线：`HETU_TEST_BACKENDS=redis uv run pytest tests/test_backend_idmap.py tests/test_backend_client.py tests/test_backend_session_basic.py tests/test_backend_session_race.py tests/test_system_executor.py -q` 全绿。
- [ ] 先写测试 `test_get_absent_unique_fields(mod_item_model)`：空 idmap → `{}`；只有 `mark_absent` 没缓存行
...
- [ ] `uv run pytest tests/test_backend_idmap.py -q` 全绿；lint；commit `perf(idmap): 新增 get_absent_unique_fields，供 commit 判定 unique 冲突是否为竞态`。
...
- [ ] **改 `test_redis_commit_payload` 期望（先红）**：测试开头加
  ...
  code 全为 `"UNIQUE"`。
- [ ] **新增 `test_redis_commit_check_codes(mod_item_model)`**（同文件，mock `lua_commit` 抓 payload）：
...
- [ ] **Lua**（`VER`/`EX`、Phase 2/3 不动；`local next` 不再用可删）：
...
- [ ] **`client.py`**：import 加 `UniqueViolation`；`commit` docstring 的 Exceptions 段改为两条（RaceCondition：
...
- [ ] **新增 Lua 回显/去重用例 `test_redis_lua_check_codes`**（`@use_redis_family_backend_only`，需 Docker）：
...
- [ ] **`test_unique_commit_race` 改期望**：两处 `pytest.raises(RaceCondition, match="UNIQUE")` →
...
- [ ] 验证：`HETU_TEST_BACKENDS=redis uv run pytest tests/test_backend_client.py tests/test_backend_session_basic.py tests/test_backend_session_race.py tests/test_system_executor.py -q` 全绿；
...
- [ ] **先写测试 `test_update_to_value_set_by_concurrent_self_update_is_race(item_ref, mod_auto_backend)`**
...
- [ ] **实现**：import 加 `UniqueViolation`、`AsyncConnection`；新增：
...
- [ ] 验证：`HETU_TEST_BACKENDS=sqlite uv run pytest tests/test_backend_session_race.py tests/test_backend_session_basic.py tests/test_backend_client.py tests/test_system_executor.py -q` 全绿；
...
- [ ] **重写 `test_insert_unique`**：删 141-144 行无效 mock（属性名不存在，从未生效）；本地重复段保持在
...
- [ ] **新增 4 个规则用例**（import 改 `from hetu.data.backend import Backend, RaceCondition, UniqueViolation`；
...
- [ ] **改 `repo.py`**：`from .base import RowFormat, UniqueViolation`（`RaceCondition` 不再使用，去掉，否则 F401）；
...
- [ ] **等值 `range` 读空也登记 absent**（用户拍板：与 `get` 对称，保住"用 range 做点查再写"的写法）。
...
- [ ] `test_insert_after_get_none_is_race` 的 docstring/注释"insert 时"改"commit 时"，断言不动。
- [ ] 验证：`HETU_TEST_BACKENDS=redis uv run pytest tests/test_backend_session_basic.py tests/test_backend_session_race.py tests/test_system_executor.py tests/test_system_future.py tests/test_headless.py tests/test_testing_sandbox.py -q` 全绿
...
- [ ] `UniqueViolation` docstring 重写：判定在两处——`insert/update` 只查本地（同事务重复立即抛）、`commit()`
...
- [ ] `hetu/headless.py:331` "get / range / unique 预检" → "get / range"。
- [ ] 文档：`docs/zh/_index.md` 96 行后加子项"unique 冲突在提交时检查：本事务曾 `get`（或等值 `range`）观察其
  ...
  这几处英文手改。）
- [ ] `uv run python scripts/gen_api_docs.py`；`git diff --stat docs/api` 应只涉及 `exceptions.md`、`system.md`、`headless.md`。
- [ ] spec `docs/superpowers/specs/2026-09-20-unique-check-at-commit-design.md`：§3.5 "`get()` 不动"改为
...
- [ ] 全量回归：`HETU_TEST_BACKENDS=redis,redis_cluster,valkey,postgres,sqlite,mariadb uv run pytest tests -q`
