...
class TestBackend(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_basic(
        ...
        np.testing.assert_array_equal(
            (await item_data.direct_query("id", -np.inf, +np.inf)).id, [1, 2, 3]
        )
        ...
        async with backend.transaction(1) as trx:
            ...
            np.testing.assert_array_equal(
                (await tbl.query("name", "Item11", "Item12")).time, [11, 12]
            )
            # query one row
        ...
        np.testing.assert_array_equal(
            (await item_data.direct_query("name", "Item11", "Item12")).time, [11, 12]
        )
        ...
        np.testing.assert_array_equal(
            (await item_data.direct_query("owner", 911)).owner, 911
        )
        row_ids = await item_data.direct_insert(owner=912, time=912)
        np.testing.assert_array_equal(
            (await item_data.direct_get(row_ids[0])).owner, 912
        )
        np.testing.assert_array_equal(
            (await item_data.direct_query("owner", 912)).owner, 912
        )
        ...
        async with backend.transaction(1) as trx:
            ...
            self.assertEqual(row.name, "updated")
            self.assertEqual(
                (await tbl.query("owner", row.owner, limit=30)).shape[0], 1
            )
            ...
            self.assertEqual(
                (await tbl.select("updated", where="name")).name, "updated"
            )
            self.assertEqual(await tbl.select(old_name, where="name"), None)
            self.assertEqual(
                len(await tbl.query("id", -np.inf, +np.inf, limit=999)), 28
            )
        async with backend.transaction(1) as trx:
            tbl = item_data.attach(trx)
        ...
        async with backend.transaction(1) as trx:
            tbl = item_data.attach(trx)
            self.assertEqual(
                len(await tbl.query("id", -np.inf, +np.inf, limit=999)), 26
            )
        ...
        async with backend.transaction(1) as trx:
            ...
            self.assertEqual(
                len(await tbl.query("id", -np.inf, +np.inf, limit=999)), 27
            )
        # 测试添加再删除
        async with backend.transaction(1) as trx:
            tbl = item_data.attach(trx)
        ...
        async with backend.transaction(1) as trx:
            tbl = item_data.attach(trx)
            np.testing.assert_array_equal(
                (await tbl.query("id", 6, 9, limit=999)).id, [6, 8, 9]
            )
            self.assertEqual(
                len(await tbl.query("id", -np.inf, +np.inf, limit=999)), 27
            )
        ...
        async with backend.transaction(1) as trx:
            tbl = item_data.attach(trx)
            self.assertEqual(
                len(await tbl.query("id", -np.inf, +np.inf, limit=999)), size
            )
    ...
    async def test_migration(
        ...
        class ItemNew(BaseComponent):
            ...
            level: np.int8 = property_field(1, unique=False, index=False)
            time: np.int64 = property_field(0, unique=True, index=True)
            name: "U6" = property_field("", unique=True, index=False)
            used: bool = property_field(False, unique=False, index=True)
    ...
    async def test_flush(
        ...
        async with backend.transaction(1) as trx:
            tbl = temp_data.attach(trx)
            self.assertEqual(
                len(await tbl.query("id", -np.inf, +np.inf, limit=999)), 25
            )
        ...
        async with backend.transaction(1) as trx:
            tbl = temp_data.attach(trx)
            self.assertEqual(len(await tbl.query("id", -np.inf, +np.inf, limit=999)), 0)
